...
pref("extensions.zotero.watchFolder.metadataFallback", true);  // when the online recognizer can't identify a PDF, create a parent registry item anyway: conservative page-1 DOI/arXiv/ISBN lookup, else a minimal filename-titled parent. Ensures no import is left a bare orphan attachment.
...
pref("extensions.zotero.watchFolder.diskDeleteSync", "ask");  // "ask" | "auto" | "never" — when a file is deleted from the watch folder, what happens to the matching Zotero item(s). "ask" (v2.8.2 default) prompts (Move to Trash / Keep / Delete Permanently, with an "always do this" checkbox); "auto" silently moves the item(s) to the Bin and shows an after-the-fact popup (legacy behavior); "never" leaves Zotero untouched. Fail-safe: no UI → keep. "Delete Permanently" is honored per-batch only and is never persisted as the standing value (downgrades to "auto"). No-op in Mode 1; consumed by v2.1+.

// v2.0 sync model — sync root + mode
pref("extensions.zotero.watchFolder.scopeMode", "library");  // 'library' (whole-library mirror — default since 2.7.0) | 'collection' (legacy single sync-root, internal fallback only; not offered in the wizard).
