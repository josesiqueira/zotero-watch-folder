...
Plugin ID: `watch-folder@zotero-plugin.org`. Version lives in `package.json` AND `manifest.json` — keep them in sync. **Current: `2.4.1`** (patch). Wizard color fix (pinned every color in `setupWizard.xhtml` so headings render dark-on-cream regardless of system theme) + two LOW security hardenings (ReDoS caps on `smartRules.matchesRegex`; proto-pollution hygiene via `sanitizeUntrustedKeys` applied in `trackingStore.load()` and `smartRules.loadRules()`). 569 unit tests.
