...
Plugin ID: `watch-folder@zotero-plugin.org`. Version lives in `package.json` AND `manifest.json` — keep them in sync. **Current: `2.3.1`** (stable). Adds two security hardenings on top of 2.3.0 — path-traversal defense against unsafe Zotero collection names, and symlink-skip in the recursive scanner. 552 unit tests.
