...
Plugin ID: `watch-folder@zotero-plugin.org`. Version lives in `package.json` AND `manifest.json` — keep them in sync. **Current: `2.4.0`** (minor — new UI feature). Lands the C1 full setup wizard as a single XHTML window (`content/setupWizard.xhtml` + `content/setupWizard.js`), replacing the v2.2/2.3 modal-sequence wizard. The modal sequence is kept as a fallback when the XHTML window fails to open (chrome not registered, embedded context, etc.) so functional coverage is identical. 555 unit tests.
...
- **Mode gates** — four layers:
  ...
  - `mirrorExecutor._deleteFolder`: Mode 2 warn-only (state flip to OUT_OF_SCOPE_SUPPRESSED + warningSink). Mode 3 recursive-moves the folder into `.zotero-watch-trash/<rel>` (with `bulkGuard` confirm when >10 files or >20% of tree affected), drops collection + child file records.
