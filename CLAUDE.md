...
Plugin ID: `watch-folder@zotero-plugin.org`. Version lives in `package.json` AND `manifest.json` — keep them in sync. **Current: `2.6.3`** (patch — launch-readiness: data-safety fixes + docs revamp; NOT yet released — last GitHub release is **2.6.2**, which already shipped the settings-UX cards + the public `docs/` folder, so 2.6.3 adds the audit's data-safety fixes + the README/contributors revamp on top). v2.6.3 came out of a multi-agent deploy-readiness audit (HOLD→GO) and lands four data-safety fixes + three hardenings on top of the 2.6.2 prefs-UX/docs work:
